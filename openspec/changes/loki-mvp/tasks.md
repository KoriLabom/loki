...
- [ ] 11.4 Con la auditoría de privacidad limpia sobre el diff real, abrir un pull request de la rama `loki-mvp` contra `main` en el repo público `loki` (ya creado y pusheado) con un resumen del cambio; verificar que el diff del PR no contiene `.env`, `config.local.yaml` ni rutas de usuario, y no mergear sin confirmación explícita del usuario
